...
import { useGetAdminLoadsQuery, useGetLoadByUserQuery } from '../../redux/loadApi';
import { useSelector } from 'react-redux';
import { FlexColumn, FlexRow } from '../UI';
import { Spinner, Avatar, AvatarGroup, Input } from '@chakra-ui/react';
...
const AdminTab = () => {
  ...
  const { data: loads, isSuccess, isLoading, isError, error } = useGetAdminLoadsQuery(user?._id);

  console.log(`Loads by ${user?.username}: `, loads);
  ...
  return (
    <div>
      ...
      {loads?.length === 0 && <h1 className="text-xl text-center">No loads have been created by you</h1>}
      {content}
    </div>
  );
};
