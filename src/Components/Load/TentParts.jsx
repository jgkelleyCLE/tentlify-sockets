...
const TentParts = ({ item }) => {
  ...
  const [parts, setParts] = useState(JSON.parse(JSON.stringify(item.parts))); // Create a deep copy of item.parts
  const [
    updateTentPart,
  ...
  const handleCompletePart = async (part) => {
    ...
    try {
      await updateTentPart(updatedPart).unwrap();
      toast({
        title: 'Part updated.',
        description: 'The part has been successfully updated.',
        status: 'success',
        duration: 5000,
        isClosable: true,
      });
